...
{% macro sybaseiq__alter_column_type(relation,column_name,new_column_type) -%}
...
{% macro sybaseiq__check_schema_exists(information_schema,schema) -%}
...
{% macro sybaseiq__create_schema(relation) -%}
'''Creates a new schema in the  target database, if schema already exists, method is a no-op. '''
  {%- call statement('create_schema') -%}
    create user {{ relation.without_identifier().include(database=False) }}
  {%- endcall -%}
{% endmacro %}
...
{% macro sybaseiq__drop_relation(relation) -%}
...
*/
  {%- call statement('drop_schema') -%}
    drop user {{ relation.without_identifier().include(database=False) }}
  {%- endcall -%}
{% endmacro %}

{% macro sybaseiq__drop_schema(relation) -%}
...
{% macro sybaseiq__get_columns_in_relation(relation) -%}
...
{% macro sybaseiq__list_relations_without_caching(schema_relation) -%}
'''creates a table of relations withough using local caching.'''
{% endmacro %}

{% macro sybaseiq__list_schemas(database) -%}
...
{% call statement('list_schemas', fetch_result=True, auto_begin=False) %}
    select 'test' as nspname
  {% endcall %}
  {{ return(load_result('list_schemas').table) }}
{% endmacro %}

{% macro sybaseiq__rename_relation(from_relation, to_relation) -%}
...
{% macro sybaseiq__truncate_relation(relation) -%}
...
{% macro sybaseiq__current_timestamp() -%}
